#!/usr/bin/env bash
#
# gen-certs.sh — gera a CA local e o certificado de servidor do broker
# `mosquitto-secure` (grupo de controle MQTTS do experimento).
#
...
# --------------------------------------------------------------------------- #
# 3. Verificação (smoke test embutido)
# --------------------------------------------------------------------------- #
...
key_mod="$(openssl rsa  -in "${CERT_DIR}/server.key" -noout -modulus 2>/dev/null)"
crt_mod="$(openssl x509 -in "${CERT_DIR}/server.crt" -noout -modulus 2>/dev/null)"
[[ "${key_mod}" == "${crt_mod}" ]] || fail "server.key não corresponde a server.crt."
...
[gen-certs] Próximos passos:
  ...
  - clientes (mosquitto_sub/pub, NestJS, ESP32) devem confiar em ca.crt;
  - ca.key e server.key são segredos locais e estão no .gitignore.
EOF
